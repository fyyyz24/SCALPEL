...
from .medical_data import MedicalJsonDataset, create_medical_dataset
...
def get_medical_dataset(args, preprocess_fn, is_train=False, tokenizer=None,
                        input_filename=None, img_root=None):
    """Load a medical dataset with ANAO metadata support."""
    assert input_filename
    medical_dataset_name = getattr(args, 'medical_dataset_name', 'mimic_cxr')
    metadata_path = getattr(args, 'medical_metadata_path', None)
    img_feature_path = getattr(args, 'img_feature_path', None)
    text_feature_path = getattr(args, 'text_feature_path', None)

    dataset = create_medical_dataset(
        dataset_name=medical_dataset_name,
        input_filename=input_filename,
        transforms=preprocess_fn,
        img_root=img_root or args.img_root,
        img_feature_path=img_feature_path,
        text_feature_path=text_feature_path,
    ...
    # Use custom collate for 4-tuple (image, text, anat, neg) batches
    dataloader = DataLoader(
        dataset,
        batch_size=args.batch_size,
        shuffle=shuffle,
...
def get_dataset_fn(data_path, dataset_type):
    ...
    elif dataset_type == "medical_json":
        return get_medical_dataset
...
def get_data(args, preprocess_fns, epoch=0, tokenizer=None):
    ...
    elif args.train_data and args.dataset_type == "medical_json":
        data["train"] = get_medical_dataset(
            args, preprocess_train, is_train=True, tokenizer=tokenizer,
            input_filename=args.train_data, img_root=args.img_root)
    ...
    if args.val_data:
        ...
        if ds_type == "medical_json":
            data["val"] = get_medical_dataset(
                args, preprocess_val, is_train=False, tokenizer=tokenizer,
                input_filename=args.val_data, img_root=args.img_root)
        else:
            data["val"] = get_dataset_fn(args.val_data, ds_type)(
                args, preprocess_val, is_train=False, tokenizer=tokenizer)
