...
from eva_clip.anao_loss import AnaOLoss
...
def train_one_epoch(model, data, epoch, optimizer, scaler, scheduler, args, tb_writer=None):
    ...
    model.train()
    use_anao = getattr(args, 'use_anao_loss', False)
    if use_anao:
        loss = AnaOLoss(
            anat_weight=getattr(args, 'anao_anat_weight', 0.1),
            neg_weight=getattr(args, 'anao_neg_weight', 0.1),
    ...
    for i, batch in enumerate(dataloader):
        ...
        if use_anao and len(batch) == 4:
            images, texts, anat_labels, neg_labels = batch
            # DataLoader default collate is inconsistent for mixed tuple fields.
            # Handle list, list-of-tensors, and already-a-tensor cases.
            if not isinstance(anat_labels, torch.Tensor):
                try:
                    anat_labels = torch.tensor(anat_labels, dtype=torch.long)
                except ValueError:
                    anat_labels = torch.stack([a if isinstance(a, torch.Tensor) else torch.tensor(a) for a in anat_labels]).long()
            if not isinstance(neg_labels, torch.Tensor):
                try:
                    neg_labels = torch.tensor(neg_labels, dtype=torch.float32)
                except ValueError:
                    neg_labels = torch.stack([n if isinstance(n, torch.Tensor) else torch.tensor(n) for n in neg_labels]).float()
            anat_labels = anat_labels.to(device=device, non_blocking=True)
            neg_labels = neg_labels.to(device=device, dtype=torch.float32, non_blocking=True)
        else:
            images, texts = batch
            anat_labels, neg_labels = None, None
        ...
        with autocast():
            ...
            if use_anao:
                total_loss, acc = loss(image_features, text_features, logit_scale,
                                       anat_labels, neg_labels)
            else:
                total_loss, acc = loss(image_features, text_features, logit_scale)
            clip_loss = total_loss.clone().detach()

        loss_list_isnan = torch.isnan(total_loss) or torch.isinf(total_loss)
        if loss_list_isnan or torch.isinf(total_loss):
            logging.info(f" ==================== loss_isnan = {loss_list_isnan},  loss_isinf = {torch.isinf(total_loss)} ==================== ")
        ...
        if is_master(args) and (i % args.log_every_n_steps == 0 or batch_count == num_batches_per_epoch):
            ...
            loss_anat_val = acc.get("anatomy_loss", 0)
            loss_neg_val = acc.get("negation_loss", 0)
            ...
            if use_anao:
                log_parts.append(f"Loss(Anat): {loss_anat_val:#.5g} ")
                log_parts.append(f"Loss(Neg): {loss_neg_val:#.5g} ")
            log_parts += [
            ...
            if use_anao:
                log_data["loss_anat"] = loss_anat_val
                log_data["loss_neg"] = loss_neg_val
...
def evaluate_iter(model, data, iter_nums, epoch, args, tb_writer=None):
    ...
    if 'val' in data:
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                # Handle both 2-tuple (img, txt) and 4-tuple (img, txt, anat, neg)
                if len(batch) >= 4:
