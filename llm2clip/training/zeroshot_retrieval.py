...
def retrieval_eval(model, data, epoch, args):
    ...
    if 'mimic_cxr' in data:
        logging.info('Starting MIMIC-CXR retrieval.')
        results = retrieval_global(model, data['mimic_cxr'].dataloader, tokenizer, args.device, args.precision)
        for key in results.keys():
            collect_results['mimic_cxr/'+ key] = results[key]
    if 'iu_xray' in data:
        logging.info('Starting IU X-Ray retrieval.')
        results = retrieval_global(model, data['iu_xray'].dataloader, tokenizer, args.device, args.precision)
        for key in results.keys():
            collect_results['iu_xray/'+ key] = results[key]
