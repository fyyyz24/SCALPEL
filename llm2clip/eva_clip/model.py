...
def _build_text_tower(
    ...
    if isinstance(text_cfg, dict):
        text_cfg_raw = text_cfg
        # Strip keys unknown to CLIPTextCfg (llm_model_name_or_path, llm_peft_path)
        import dataclasses as _dc
        known_keys = {f.name for f in _dc.fields(CLIPTextCfg)}
        filtered = {k: v for k, v in text_cfg.items() if k in known_keys}
        text_cfg = CLIPTextCfg(**filtered)
    ...
    if text_cfg.hf_model_name:
        ...
        # Extract LLM-specific config from raw dict (not captured by CLIPTextCfg)
        llm_path = text_cfg_raw.get('llm_model_name_or_path', 'meta-llama/Llama-3.1-8B-Instruct') if text_cfg_raw else 'meta-llama/Llama-3.1-8B-Instruct'
        llm_peft = text_cfg_raw.get('llm_peft_path', None) if text_cfg_raw else None
        text = LLM2VecTextTransformer(text_proj, model_name_or_path=llm_path, peft_path=llm_peft)
    else:
        act_layer = QuickGELU if quick_gelu else nn.GELU
        norm_layer = LayerNorm
