...
class ModelArguments:
    ...
    extra_model_name_or_path: Optional[List[str]] = field(
        default=None,
        metadata={"help": "The extra model checkpoint for weights initialization."}
    )
...
def main():
    ...
    # Check if 4-bit quantization is requested via environment variable
    use_4bit = os.environ.get("CRC_USE_4BIT", "").lower() in ("1", "true", "yes")
    if use_4bit:
        load_kwargs = {
            "load_in_4bit": True,
        ...
        torch_dtype = None  # 4-bit overrides torch_dtype
    ...
    model = model_class.from_pretrained(
        ...
        low_cpu_mem_usage=False if use_4bit else model_args.low_cpu_mem_usage,
